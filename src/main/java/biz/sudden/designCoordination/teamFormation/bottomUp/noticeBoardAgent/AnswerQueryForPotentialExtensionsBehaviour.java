 ...
 * 
 *         As such it uses the SL Interpreter from below to answer these queries
 *         together with a set of test classes within this folder which are
 *         concrete implementations of the predicates..
 */

@SuppressWarnings("serial")
public class AnswerQueryForPotentialExtensionsBehaviour extends
		OneShotBehaviour {

	private ACLMessage queryIn;
	private NoticeBoardAgent myNBAgent;

	public AnswerQueryForPotentialExtensionsBehaviour(NoticeBoardAgent myAgent,
			ACLMessage Query) {
		super(myAgent);
		this.queryIn = Query;
		myNBAgent = myAgent;
		...
		if ((queryCompiled != null) && (IREfromQuery != null)) {
			try {
				...
				for (PotentialExtension potentialExtension : matchedResults) {
					// TODO - convert potential extension to AbsConcept & store
					// it.
					results
