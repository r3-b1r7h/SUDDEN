 ...
 *         given abstract types within it's goal state.
 * 
 *         Used to eg filter for noticeboards producing a given type of object.
 */
...
public class ProducesAbstractTypesTest extends ConcreteCompiledPredicate {
	...
	public ProducesAbstractTypesTest(AbsPredicate predToConvert,
			Ontology ontology) {
	...
	public boolean evaluateMatch(Object toBeMatched) {
		...
		AbsConcept stateToTest = (AbsConcept) noticeboardDescription
				.getAbsObject(CreateNoticeboardOntology.NOTICEBOARD_DESCRIPTION_GOAL_STATE);
		 ...
		 * only for now?
		 * 
		 * This currently returns tests only if querying for a more general
		 * type....
		 */
